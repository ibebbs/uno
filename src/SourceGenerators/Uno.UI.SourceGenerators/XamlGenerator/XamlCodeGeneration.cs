...
#pragma warning restore 649 // Unused member
		...
		public XamlCodeGeneration(GeneratorExecutionContext context)
		{
			...
			_legacyTypes = context
				.GetMSBuildItems("LegacyTypes")
				.ToList()
				.ToDictionary(k => k.Identity, fullyQualifiedName => fullyQualifiedName.Identity.Split('.').Last());
...
#if false // CS9 TODO
			_uiAutomationMappings = context.GetMSBuildItems("CustomUiAutomationMemberMapping")
				.Select(i => new
				{
			...
			_isWasm = context.GetMSBuildPropertyValue("DefineConstants")?.Contains("__WASM__") ?? false;
		}
